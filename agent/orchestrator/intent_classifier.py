...
# Implicit travel intent – emotional or life-event messages with travel desire
_IMPLICIT_TRAVEL_PATTERNS = [
    re.compile(r'想.{0,4}(去|出去|回|走走|出发|看看)'),  # desire to go
    re.compile(r'(散心|放松一下|冷静几天|透透气|换个环境)'),  # emotional travel motivation
    re.compile(r'(风景|大自然|海边|山里|乡下|城市看看)'),  # scenic implicit travel
    re.compile(r'(犒劳|庆祝|纪念|聚会).{0,6}(自己|团队|我们|一下)'),  # celebration
    re.compile(r'带.{0,4}(出去|去|散心|旅|玩)'),  # take someone on a trip
    re.compile(r'一起去|自己出去|好好(放松|玩|休息)'),  # explicit go-together / solo
    re.compile(r'为自己(活|走|闯|拼)'),  # self-discovery / life-change motivation
]
...
class IntentClassifier:
    ...
    WEIGHTS = {
        ...
        "implicit_travel": 0.4,       # Implicit travel intent (emotional/life-event)
    }
    ...
    def classify(
        ...
        # Feature 8: Implicit travel intent (emotional/life-event + travel desire)
        if any(p.search(msg) for p in _IMPLICIT_TRAVEL_PATTERNS):
            score += self.WEIGHTS["implicit_travel"]
